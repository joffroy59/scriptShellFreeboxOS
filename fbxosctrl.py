...
gAppDesc = {
    "app_id": "fr.freebox.joffroy.1",
    "app_name": "joffroy App 1",
    "app_version": "0.0.1",
...
its exposed REST API """
    ...
    def __init__(self, fbxAddress="http://mafreebox.freebox.fr",
                 regSaveFile="fbxosctrl_registration.app_token"):
        """ Constructor """
...
LCD screen. This command shall be executed only once. """
    ...
    def getWifiStatus(self):
        ...
        headers = {
            'X-Fbx-App-Auth': self.sessionToken, 'Accept': 'text/plain'}
        url = self.fbxAddress + "/api/v2/wifi/ap/0"
        # GET
        log("GET url: %s" % url)
        ...
        #log("Obj resp: %s" % resp)
        isOn = True
        log("errer %s" % resp['result']['status']['state'])
        if True == resp['success']:
